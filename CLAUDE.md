...
Unlike the pure layers below it, a strategy **acts**: `Strategy` has `evaluate(&mut self, Input, &mut dyn Wallet<Symbol>)` and `reset()` (associated `Input`/`Symbol`). Each bar it reads the input and opens/scales/closes positions on the **wallet handed to it** — it returns nothing and holds no portfolio state. There is deliberately **no rule engine / policy traits / generic `(signal, action)` strategy type in the crate**: every strategy is its own concrete type implementing the trait (a struct holding its signals/indicators; `evaluate` advances them all then calls wallet methods). This evolved through several discarded designs (pure orders-out, policy objects, a `(signal, action)` `RuleStrategy`) — do not reintroduce those without being asked. The crate *does* ship `src/strategies/` — a **catalogue of classical single-asset strategies** (`MaCrossover`, `MacdCrossover`, `DonchianBreakout`, `RsiReversal`, `BollingerReversion`, `ZScoreReversion`, `MomentumRoc`, `ObvTrend`, `AdxTrendFilter`, `KeltnerBreakout`, … grouped under `trend`/`mean_reversion`/`momentum`/`volume`/`composite`). These are concrete worked examples, each generic over `Sym: Clone` with `Input = Candle`; they are **not** a framework — shared code is limited to tiny mechanical helpers (`is_flat`/`is_long`/`is_short`, `enter_all_in`) in `strategies/mod.rs`, never a decision/rule abstraction. Long/flat strategies open on an entry edge and flatten on an exit edge; always-in long/short ones flip via flatten-then-reopen all-in (`enter_all_in`) so `funds_frac` sizing stays exact.
